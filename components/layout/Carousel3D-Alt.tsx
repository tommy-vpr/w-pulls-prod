...
// Simplified plastic case - just a glass-like shell
function PlasticCase({
  ...
  const caseDepth = 0.006; // Much thinner case
  const cornerRadius = 0.035; // More visible rounding
  ...
  return (
    <group>
      {/* Main case shell - single rounded box with glass material */}
      <RoundedBox
      ...
      {/* Holographic shimmer overlay on front */}
      <mesh
...
// Simplified card with proper thin dimensions
function Card({ url, cardBack, ...props }: CardProps) {
  const groupRef = useRef<THREE.Group>(null);
  const [hovered, setHovered] = useState(false);

  const frontTexture = useTexture(url);
  const backTexture = useTexture(cardBack);

  // Card dimensions - thin like a real trading card
  const cardWidth = 0.7;
  const cardHeight = 0.98;
  const cardThickness = 0.002; // Very thin
  const cornerRadius = 0.03; // Visible rounded corners
  ...
  return (
    <group {...props}>
      ...
      >
        ...
        {/* Single card body with both faces textured */}
        <RoundedBox
        ...
        {/* Front face texture (facing outward from carousel) */}
        ...
        {/* Back face texture (facing inward) */}
        <mesh
...
function Banner({ position = [0, -0.15, 0], texture }: BannerProps) {
  ...
  useFrame((_state, delta) => {
    ...
    const scrollDelta = Number.isFinite(scroll.delta) ? scroll.delta : 0;
    ref.current.material.time.value += Math.abs(scrollDelta) * 4;
  ...
  return (
    <mesh ref={ref} position={position}>
      ...
      {/* @ts-expect-error - Custom material from util.ts extend() */}
      <meshSineMaterial
        map={loadedTexture}
        map-anisotropy={16}
        map-repeat={[30, 1]}
    ...
    <ScrollControls pages={4} infinite>
      ...
      {bannerTexture && (
        <Suspense fallback={null}>
          <Banner position={[0, -0.6, 0]} texture={bannerTexture} />
        </Suspense>
      )}
    </ScrollControls>
  );
