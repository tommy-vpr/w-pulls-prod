export function verificationEmail({
  pin,
  name,
}: {
  pin: string;
  name: string;
}) {
  return `
<!DOCTYPE html>
<html>
<head>
  <meta charset="utf-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1.0" />
  <title>Your Verification Code – WPulls</title>
</head>
<body style="background-color: #030812; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; margin: 0; padding: 0;">
  <div style="max-width: 600px; margin: 0 auto; padding: 20px;">

    <!-- Header -->
    <div style="text-align: center; padding: 30px 0;">
      <p style="font-size: 28px; font-weight: bold; color: #00ffff; letter-spacing: 4px; margin: 0;">WPulls</p>
      <p style="font-size: 10px; color: rgba(0,255,255,0.5); letter-spacing: 3px; margin: 4px 0 0 0;">CARD SYSTEM</p>
    </div>

    <!-- Verification Card -->
    <div style="background: linear-gradient(135deg, rgba(0,255,255,0.1), rgba(255,0,255,0.05)); border: 1px solid rgba(0,255,255,0.3); border-radius: 12px; padding: 30px; text-align: center;">
      <h1 style="font-size: 22px; color: #ffffff; margin: 0 0 12px 0;">
        Your verification code
      </h1>
      <p style="font-size: 15px; color: rgba(255,255,255,0.7); margin: 0 0 24px 0;">
        Hey ${name}, enter this code to verify your email and activate your account.
      </p>

      <!-- PIN Code -->
      <div style="background: rgba(0,0,0,0.4); border: 2px solid rgba(0,255,255,0.4); border-radius: 12px; padding: 20px; margin: 0 auto; max-width: 280px;">
        <p style="font-size: 36px; font-weight: bold; color: #00ffff; letter-spacing: 12px; margin: 0; font-family: 'Courier New', monospace;">
          ${pin}
        </p>
      </div>

      <p style="font-size: 12px; color: rgba(255,255,255,0.5); margin: 24px 0 0 0;">
        This code expires in 15 minutes.
      </p>
    </div>

    <!-- Footer -->
    <div style="text-align: center; padding: 30px 0;">
      <p style="font-size: 11px; color: rgba(255,255,255,0.3); margin: 0;">
        If you didn't create an account, you can safely ignore this email.
      </p>
      <p style="font-size: 11px; color: rgba(255,255,255,0.3); margin: 8px 0 0 0;">
        © ${new Date().getFullYear()} WPulls. All rights reserved.
      </p>
    </div>

  </div>
</body>
</html>
`;
}
